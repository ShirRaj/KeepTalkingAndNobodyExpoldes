...
void process_logic() {
  int right_wires = 1;
  int wrong_wires = 0;
  int right_switches = 1;
  int wrong_switches = 0;

  for (int i =0; i < NUM_SWITCHES; i++) {
    if(correct_wires[i] >=0 && digitalRead(BASE_SCREW_PIN+i)==HIGH){
      right_wires = 0;
    }
  }
  for(int i = 0; i < NUM_SWITCHES; i++) {
  ...
  for (int i = 0; i < NUM_SWITCHES; i++) {
    if(correct_plus_switches[i] >= 0) {
      if(plus_switch_state[correct_plus_switches[i]] == 0)
        right_switches=0;
      else
        plus_switch_state[correct_plus_switches[i]] = 0;
    }
    if(correct_minus_switches[i] >= 0) {
      if(minus_switch_state[correct_minus_switches[i]] == 0)
        right_switches=0;
      else
        minus_switch_state[correct_minus_switches[i]] = 0;    
    }
  }

  for (int i = 0; i < NUM_SWITCHES; i++) {
    if(plus_switch_state[correct_plus_switches[i]]) {
      wrong_switches = 1;
    }
    if(plus_switch_state[correct_minus_switches[i]]) {
      wrong_switches = 1;
    }
  }
...
boolean check_penalty() {
  ...
  if (correct_wires==1 && correct_switches==0){
    return true; 
  }
  return false;
}
...
boolean check_solved() {
  if (correct_wires==1 && correct_switches==1){
    return true;
  }
  return false;
}
...
boolean check_if_ready() {
  ...
  for(int i = 0; i < NUM_SWITCHES; i++) {
    if(read_pin(BASE_MINUS_PIN+2*i)==LOW || read_pin(BASE_PLUS_PIN+2*i)==LOW) {
      return false;
    }
  }
  return true;
